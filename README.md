...
This is not a official CI library or @chriskacerguis repo.

This just is an adjustment from him library

## How to

CI needs to be extended, any functions are not available for CI2, and other ones needs to change property access.

* Put MY_ core files on your app/core directory.
* Put common_helper on app/helpers. Pref. setup on config/autoload. Some functions was added to core/Commons.php on CI3.
* Put Rest_Controller library and Format on your app libs folder

Make petitions to /api/example/users/:id with Advanced Rest Client (Chrome) and HTTP Requester (firefox) to try.

If find a bug, please leave your comment.
...
### Not works, this is the reason for this
